...
void fdCAN_Send_Data(uint32_t ide, uint32_t id, uint8_t dlc, const uint8_t *data)
{
    ...
    if(HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2, &tx, data)!=HAL_OK)
    {
        return;
    }
}

static void Receive_to_OsQueue(osMessageQueueId_t* osMessageQueue,FDCAN_HandleTypeDef *hfdcan,FDCAN_RxHeaderTypeDef* rx)
{
    uint8_t data[8] = {0};
    CanMsg_t Msg = {0};
    ...
    osMessageQueuePut(zdrive_can_rx_queue, &Msg, 0, 0);
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan,uint32_t RxFifo0ITs)
{
    ...
    ZdriveReceiveMsg msg={0};

    if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &rx, data) != HAL_OK)
        return;
    ZdriveReceive(rx,data,0);
}
